...
TP_SIZE=32
PP_SIZE=32 # num_layer % pp_size == 0
...
CHECKPOINT_PATH=/groups/gcf51174/checkpoints/megatron-deepspeed/mpirun/175b/${NNODES}node-${WORLD_SIZE}gpu-dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}
...
mpirun -np $WORLD_SIZE --npernode $GPUS_PER_NODE \
  ...
  --wandb-name "gpt2_175b_${NNODES}node_dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun"
