...
CHECKPOINT_PATH=/groups/gcf51174/checkpoints/megatron-deepspeed/mpirun/13b/${NNODES}node-${WORLD_SIZE}gpu-dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}
...
mpirun -np $WORLD_SIZE --npernode $GPUS_PER_NODE \
  ...
  --wandb-name "gpt2_13b_${NNODES}node_dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun"
